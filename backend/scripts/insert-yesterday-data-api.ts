...
async function enableSupabase() {
  try {
    const { data } = await postJSON(`${API_BASE}/data-source/toggle`, { enabled: true });
    if (data.success) {
      console.log('✓ 已启用 Supabase 数据源');
      return true;
    }
    console.error('✗ 启用 Supabase 失败:', data.error);
    return false;
  } catch (error: any) {
    console.error('✗ 启用 Supabase 失败:', error.message);
    return false;
  }
}

async function checkYesterdayData(date: string) {
  try {
    const { data } = await getJSON(`${API_BASE}/daily-reports/${date}`);
...
async function insertDailyReports(date: string) {
  ...
  for (const report of reports) {
    try {
      const { data } = await postJSON(`${API_BASE}/daily-reports`, report);
      if (data.success) {
        console.log(`  ✓ 日报 ${report.business_system_id}`);
      } else {
        console.log(`  ✗ 日报 ${report.business_system_id}: ${data.error}`);
      }
    } catch (error: any) {
...
async function insertLogMetrics(date: string) {
  ...
  try {
    const { data } = await postJSON(`${API_BASE}/log-metrics/batch`, { metrics });
    if (data.success) {
      console.log(`  ✓ 插入 ${metrics.length} 条日志指标`);
    } else {
      console.log(`  ✗ 指标插入失败: ${data.error}`);
    }
  } catch (error: any) {
...
async function insertCloudRegions(date: string) {
  ...
  try {
    const { data } = await postJSON(`${API_BASE}/cloud-regions/batch`, { regions });
    if (data.success) {
      console.log(`  ✓ 插入 ${regions.length} 个云区域`);
    } else {
      console.log(`  ✗ 云区域插入失败: ${data.error}`);
    }
  } catch (error: any) {
...
async function insertAssessments(date: string) {
  ...
  try {
    const { data } = await postJSON(`${API_BASE}/assessments/batch`, { assessments });
    if (data.success) {
      console.log(`  ✓ 插入 ${assessments.length} 条评估`);
    } else {
      console.log(`  ✗ 评估插入失败: ${data.error}`);
    }
  } catch (error: any) {
...
async function insertActionPlans(date: string) {
  ...
  try {
    const { data } = await postJSON(`${API_BASE}/action-plans/batch`, { plans });
    if (data.success) {
      console.log(`  ✓ 插入 ${plans.length} 条行动计划`);
    } else {
      console.log(`  ✗ 行动计划插入失败: ${data.error}`);
    }
  } catch (error: any) {
